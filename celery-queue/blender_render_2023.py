...
def setup_scene(cam_pos, cam_rot, actor1, actor2, arm1, arm2, plane_size=8):
    ...
    actor2_bvh_offset_y = Vector((0, -1.4, 0)) + arm2.matrix_world @ arm2.data.bones['b_root'].head_local
...
#    sky_mat.diffuse_color = (0.15, 0.4, 0.25, 1)
    sky_mat.diffuse_color = (0.8, 0.8, 0.25, 1)
...
def add_plane(prov_size):
    ...
    plane_obj.scale[0] = 1.165
    plane_obj.scale[1] = 1.25
    plane_obj.location[1] = -0.25
    mat = bpy.data.materials['FloorColor'] #set new material to variable
    mat.diffuse_color = (0.15, 0.3, 0.25, 1)
#    mat.diffuse_color = (0.125, 0.125, 0.125, 1)
...
def render_video(output_dir, picture, video, bvh1_fname, bvh2_fname, actor1, actor2, render_frame_start, render_frame_length, res_x, res_y):
    ...
    if picture:
        ...
        bpy.context.scene.render.filepath=os.path.join(output_dir, 'Main_{}_color6.png'.format(bvh1_fname))
        bpy.ops.render.render(write_still=True)
        get_camera(actor1 + '_cam')
        bpy.data.objects[actor1].children[1].hide_render = True
        bpy.data.objects[actor2].children[1].hide_render = False
        bpy.context.scene.render.filepath=os.path.join(output_dir, '{}_color6.png'.format(bvh1_fname))
        bpy.ops.render.render(write_still=True)
        get_camera(actor2 + '_cam')
        bpy.data.objects[actor1].children[1].hide_render = False
        bpy.data.objects[actor2].children[1].hide_render = True
        bpy.context.scene.render.filepath=os.path.join(output_dir, '{}_color6.png'.format(bvh2_fname))
        bpy.ops.render.render(write_still=True)
    ...
    if video:
        ...
        bpy.data.objects[actor1].children[1].hide_render = False
        bpy.data.objects[actor2].children[1].hide_render = False
        bpy.context.scene.render.filepath=os.path.join(output_dir, 'Main_{}_'.format(bvh1_fname))
...
def main():
    ...
    if bpy.ops.text.run_script.poll():
        ...
        ##################################
        ARG_BVH1_PATHNAME = SCRIPT_DIR / 'test/' / 'session27_take4_hasFingers_deep5_SL_30fps-normalized.bvh'
        ARG_BVH2_PATHNAME = SCRIPT_DIR / 'test/' / 'session27_take4_hasFingers_shallow24_SL_30fps-normalized-faced.bvh'
        ARG_AUDIO_FILE_NAME = SCRIPT_DIR / 'test/' / 'session27_take4.wav' # set to None for no audio
#        ARG_AUDIO_FILE_NAME = SCRIPT_DIR / 'test/' / 'session15_take17_shallow13.wav' # set to None for no audio
        ARG_IMAGE = True
        ARG_VIDEO = True
        ARG_START_FRAME = 0
...
#    setup_characters(BVH1_NAME, BVH2_NAME)
    setup_characters_faced(BVH1_NAME,BVH2_NAME)
    
    # 05/04/2023 fix main camera orientation, fix character rotation and personal cameras
    if ARG_MODE == "full_body":     CAM_POS = [3.25, -0.25, 1.1]
    elif ARG_MODE == "upper_body":  CAM_POS = [0, -2.45, 1.3]
    MAIN_CAM_ROT = [math.radians(90), 0, math.radians(90)]
